...
try
{
    class database
    {
        ...
        public static function readwithid(PDO $pdo ,string $table, int $id)
        {
            $tableStatement =$pdo->prepare("select * from $table where id_$table=$id");
            $tableStatement->execute();
            $tables = $tableStatement->fetchAll();
            return $tables;
        }
            ...
            $pdo=new PDO('mysql:host=localhost;dbname=e_commerce;charset=utf8',"root");
                ...
                else
                {
                    ...
                    foreach ($users_login as $user) 
                    {
                        ...
                        {
                            SESSION::setsession($email,$password);
